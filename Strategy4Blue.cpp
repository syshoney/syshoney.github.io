...
void Goalie(Environment* pEnv)
{
	...
	// 计算球预测轨迹与门线(x=215)的交点
	double pred_y = bally;
	if (A[0] != ballx) {
		double slope = (A[1] - bally) / (A[0] - ballx);
		pred_y = slope * (215 - ballx) + bally;
	}

	double target_x = 215, target_y = pred_y;
	ClampGoaliePosition(target_x, target_y);
	...
	// 对方半场：门线上跟随预测交点
	if (ballx < 180) {
		Position_Goalie(pEnv, 0, target_x, target_y, 160);
		return;
	}
	...
	if (flag >= 1) {
		if (A[1] > 120) {
			if (bally > 120 && ballx < 215) {
				double tx = 215, ty = (pred_y + 90) / 2 + 10;
				ClampGoaliePosition(tx, ty);
				Position_Goalie(pEnv, 0, tx, ty, 170);
			}
			else {
				double tx = 215, ty = 130;
				ClampGoaliePosition(tx, ty);
				Position_Goalie(pEnv, 0, tx, ty, 170);
			}
		}
			...
			ClampGoaliePosition(tx, ty);
			Position_Goalie(pEnv, 0, tx, ty, 165);
		}
		return;
	}

	// 无敌人配合时，球在己方门区且离球最近，主动拦截
	if (D < ball_player && ballx > 200 && ballx < 220 && bally > 45 && bally < 135) {
		if (D < 10) {
			Velocity(&(pEnv->home[0]), 180, 180); // 冲球
		}
		else {
			Position_Goalie(pEnv, 0, ballx, bally, 170);
		}
		return;
	}
